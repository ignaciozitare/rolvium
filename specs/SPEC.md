...
- [journal (H9)](modules/journal/SPEC.md) — Notas privadas (ni el director ni un admin las leen) y Bitácora compartida de la campaña, en el carril lateral de la mesa (sin historial de versiones en la v1)
- [adventures (H12)](modules/adventures/SPEC.md) — aventuras del director, pestaña de la mesa sólo suya: documento, escenas y encuentros; en curso, orden, archivo y borrado
...
| ✅ `campaigns` | 17 | `container` · `Campaign` · `CampaignsPort` · `campaignRules` · `CampaignsPage` — `apps/web/src/modules/campaigns/index.ts` |
| ✅ `journal` · ✅ `adventures` | — (nacieron con ella, 2026-09-19) | `apps/web/src/modules/journal/index.ts` · `apps/web/src/modules/adventures/index.ts` |
...
|---|---|---|
| `core/auth` | 29 | casi todas |
| `core/roles-permissions` · `modules/identity` | 51-71 | la mayoría |
| `core/realtime` · `core/game-system` · `core/images` · `modules/system-plenilunio` · `modules/chat` | 80-106 | las secciones del molde |
| ✅ `modules/journal` · `modules/adventures` · `modules/dice` · `modules/campaigns` · `modules/characters` | 144-391 | **ninguna** — cerrados en `feat/notas-bitacora-aventuras` (2026-09-19/21) |
| ✅ `modules/bestiary` · `core/testing` · `core/errors` | 106-177 | **ninguna** |
