...
const META = {
  ...
  EmptyState:         { resolves: 'estado vacío/error centrado con icono, texto y acciones; `tone` accent/red/amber — ámbar es «se ha roto y no es culpa tuya»', usage: '<EmptyState icon title description actions tone>' },
  ErrorBoundary:      { resolves: 'la RED: un error AL PINTARSE se queda en su trozo en vez de llevarse la pantalla entera. No atrapa eventos ni promesas. En la app se usa vestida: apps/web/src/shared/ui/SafeRegion.tsx', usage: '<ErrorBoundary label fallback={(reintentar, error) => …}>…</ErrorBoundary>' },
  ...
  Tooltip:            { resolves: 'etiqueta breve al pasar el ratón o al enfocar con teclado; sustituye al `title` nativo en botones de solo icono, tema solo vía --sys-*', usage: '<Tooltip label="Muro" placement="right">{children}</Tooltip>' },
