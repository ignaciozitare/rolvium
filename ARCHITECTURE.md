...
| `table` (H3) | `/table/:id`: the live table shell — tabs, shared-resources bar — and **the place that dresses the table with the campaign system's `VisualTheme`** (`system.theme.vars` → `--sys-*` inline on `.tb-root`; the sheet in its own window does the same in `CharacterSheetPage`) | port `TablePort` → `SupabaseTableRepo`; `container.ts`; UI `TablePage` (+ `useTable`), `SharedResourceBar`, `tabs/{SheetTab,GroupTab,SceneTab}`. ⚠ `table.css` also holds the table primitives (`tb-btn`, `tb-btn-blood`, `tb-rotulo`…) that `maps` and `characters` use — see «Known architecture debt» |
