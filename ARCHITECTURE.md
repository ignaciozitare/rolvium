...
- The map's floating panels (`BuilderPanel`, `BrushPanel`, `LightEditor`) each re-implement the same shell (header with
  grip/title/close, position, width), and sliders exist in three shapes (stacked in the Pincel, inline in the Builder
  and in `LightEditor`). `@rolvium/ui` has no floating panel and no slider, so the objects panel would be a fourth copy.
...
Proposed order, **pending the owner's OK**: extract the floating panel + slider + option buttons into `@rolvium/ui`
before building the objects panel; take Plenilunio out of `bestiary` / `dice`, move `METRES_PER_CELL` into the port and
neutralise the `table.css` defaults before a second system.
