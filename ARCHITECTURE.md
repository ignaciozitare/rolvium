...
| — | error net | `ErrorBoundary` (`@rolvium/ui`, behaviour only — its fallback is a render prop so the package stays i18n-free) dressed by `apps/web/src/shared/ui/SafeRegion.tsx` in two suits: `--sys-*` inside the table, app chrome for the full-screen last resort. Four seams: the whole app (`AppRouter`), each table tab (`TablePage`, `key={tab}`), the map and each floating panel (`SceneTab`). Catches render errors only — not event handlers, promises or the server. See `specs/core/errors/SPEC.md` | everyone |
