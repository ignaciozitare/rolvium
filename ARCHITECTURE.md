...
| `maps` (H7) | Escena: scenes, background (colour + campaign image library), walls, tokens (PC/bestiary), shared drawings, measure, focus pin, zoom/pan; DM tools behind the gold separator | ports `MapsPort`; adapter `SupabaseMapsRepo` (tables `maps_*`, bucket `backgrounds`, `postgres_changes` + broadcast for drag/pin); `container.ts` → `mapsRepo`; UI `SceneTab` (table tab `scene`), `MapCanvas` + `canvasLayers`, `Toolbar`, `StrokeBar`, `ScenesMenu`, `BackgroundPopover`, `EncounterMenu`, `CanvasControls`, `useScene` |
...
`roll`, returning `{ id, request, dice, result, rolledAt, effectsApplied?, sheet? }`), `POST /admin/users`,
