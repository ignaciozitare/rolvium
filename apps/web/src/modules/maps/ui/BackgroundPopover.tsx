...
import type { BgFit, BgTransform, ImageAsset, Scene } from '../domain/entities/Scene';
...
export function BackgroundPopover({ scene, images, onColor, onImage, onTransform, onUpload, onClose }: Props): JSX.Element {
  ...
  return (
    <div className="mp-pop mp-bgpop" role="dialog" aria-label={t('maps.bg.title')}>
      <div className="mp-pop-head">
        <span className="mp-pop-title">{t('maps.bg.title')}</span>
      ...
      <div className="mp-pop-body">
        ...
        <div className="mp-lib">
          ...
          {images?.map(img => (
            <button key={img.id} type="button" className={`mp-lib-item ${scene.bgImageUrl === img.url ? 'on' : ''}`} aria-pressed={scene.bgImageUrl === img.url} aria-label={img.name} onClick={() => onImage(img.url)}>
              <span className="mp-lib-thumb" style={{ backgroundImage: `url(${img.url})` }} /><span className="mp-lib-name">{img.name}</span>
            </button>
          ))}
          <button type="button" className={`mp-lib-item ${scene.bgImageUrl === null ? 'on' : ''}`} aria-pressed={scene.bgImageUrl === null} aria-label={t('maps.bg.none')} onClick={() => onImage(null)}>
