...
import type { AttackErrorCode, IAttackRepository, OpenAttackInput, PendingAttack } from '../../domain/attack/IAttackRepository.js';

const codeFor = (message: string): AttackErrorCode | 'DB_ERROR' =>
  /not_pending/i.test(message) ? 'NOT_PENDING'
    : /not_dm|not_member|not_authenticated/i.test(message) ? 'FORBIDDEN'
      : 'DB_ERROR';

interface Row {
  id: string; campaign_id: string; target_character_id: string; created_by: string;
  dice: number; request: RollRequest; status: PendingAttack['status'];
}

const SELECT = 'id, campaign_id, target_character_id, created_by, dice, request, status';
